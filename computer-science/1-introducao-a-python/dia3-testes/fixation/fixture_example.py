...
@pytest.fixture
def orders():
    """Nosso cenario (contexto) tem os seguintes pedidos:"""
    return [
        {"customer": "maria", "order": "pizza", "day": "terca-feira"},
        {"customer": "joao", "order": "hamburger", "day": "terca-feira"},
...
def test_get_order_frequency_per_customer_when_customer_is_joao_and_order_is_pizza(orders):
    assert get_order_frequency_per_customer(orders, "joao", "pizza") == 0


def test_get_order_frequency_per_customer_when_customer_is_maria_and_order_is_hamburger(orders):
    assert get_order_frequency_per_customer(orders, "maria", "hamburger")